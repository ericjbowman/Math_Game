...
function App(props) {
  ...
  const [wallSpeed, _setWallSpeed] = useState(props.defaultGame.wallSpeed)
  const [nextLevel, _setNextLevel] = useState(props.defaultGame.nextLevel)
  const [currentLevel, _setCurrentLevel] = useState(props.defaultGame.currentLevel)
  ...
  useEffect(() => {
    if (playerStatsRef.current.right === nextLevelRef.current) {
      tlRef.current.timeScale(1 + (currentLevelRef.current * props.defaultGame.speedScaling))
      setCurrentLevel(currentLevelRef.current + 1)
      setNextLevel(nextLevel * props.defaultGame.levelScaling)
    }
  }, [playerStats])
  ...
  function startGame() {
    ...
    setCurrentLevel(props.defaultGame.currentLevel)
    setNextLevel(props.defaultGame.nextLevel)

    document.addEventListener('keypress', onKeyPress)
    document.addEventListener('keyup', onKeyUp)

    tl = gsap.timeline({repeat: -1});
    tl.timeScale(1 + ((currentLevelRef.current - 1) * props.defaultGame.speedScaling))
  ...
  function getPlayerLane() {
    console.log('game play container', gameplayContainerRef.current)
    const gameplayContainerWidth = gameplayContainerRef.current ? gameplayContainerRef.current.offsetWidth : 0
  ...
  return (
    <div className="App">
      ...
      <div className="gameplay-container" ref={gameplayContainerRef}>
        <Player
          gameplayContainerRef={gameplayContainerRef}
          playerPhysicsRef={playerPhysicsRef}
        />
        <div
          className="wall"
          ref={wallRef}
